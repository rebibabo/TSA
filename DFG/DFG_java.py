...
def DFG_java(root_node,index_to_code,states):
    ...
    do_first_statement=[]    
    states=states.copy()
    if (len(root_node.children)==0 or root_node.type=='string') and root_node.type!='comment':      # 变量名
        idx,code=index_to_code[(root_node.start_point,root_node.end_point)]
        if root_node.type==code:    # 如果类型和内容一样，例如=，那么忽略
            return [],states    
        ...
        return sorted(DFG,key=lambda x:x[1]),states
    elif root_node.type in enhanced_for_statement:  # for (ElementType name : value) body
        name=root_node.child_by_field_name('name')
        value=root_node.child_by_field_name('value')
